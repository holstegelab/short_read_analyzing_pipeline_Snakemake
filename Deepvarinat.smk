...
rule Deepvariant_all:
    input:
        expand("{cur_dir}/{deepvarinat}/VCF/{chr}.{sample}.{mode}.vcf.gz", cur_dir = current_dir, deepvarinat = config['DEEPVARIANT'], sample = sample_names, mode = mode, chr = main_chrs),
        expand("{cur_dir}/glnexus/{chr}/{chr}_{mode}.vcf.gz", cur_dir = current_dir, mode = mode, chr = main_chrs),
        expand("{cur_dir}/glnexus_custom/{chr}/{chr}_{mode}.vcf.gz", cur_dir = current_dir, mode = mode, chr = main_chrs),
...
def get_chrom_capture_kit_bed(wildcards):
    ...
    # else:
    #     if mode == 'WES':
    #         capture_kit_chr_path_bed = config['RES'] + config['kit_folder'] + capture_kit + '_hg38/' + capture_kit + '_hg38_' + chr + '.interval_list.bed'
    # else:
    #     raise ValueError(
...
rule glnexus:
    ...
    output: vcf = os.path.join(current_dir, "glnexus", "{chr}", "{chr}_{mode}.vcf.gz")
...
#
    ...
    #     container: 'docker://google/deepvariant:1.4.0'
    #     benchmark:
    #         os.path.join(current_dir, config['BENCH'],"{chr}_{sample}.{mode}.deepvariant.txt")
    #     log: os.path.join(current_dir, config['LOG'],"{chr}_{sample}.{mode}.deepvariant.log")
    #     shell:
    #         # singularity run -B /usr/lib/locale/:/usr/lib/locale/ docker://google/deepvariant:"1.4.0" \
    #         """
