    ...
    shell:
            """
                 gatk MergeMutectStats --stats {input.orig} --stats {input.shift} -O {output.merged_stat} 
                              ...
                              bcftools norm -m- -Ov  {output.filtred_vcf} |  bcftools norm -d exact -O v -o {params.filtred_norm_vcf} - &&               bgzip {params.filtred_norm_vcf} && 
    ...
    shell:
            """
             ...
             ((gatk Mutect2 -ERC BP_RESOLUTION -R {params.mt_ref} -L chrM --mitochondria-mode -I {input.bam} -O {output.vcf_BP})   
              ...
              bcftools norm -Ov -m- {output.merged_vcf_BP} | bcftools norm -d exact -o {output.merged_vcf_BP_norm} -O z - && tabix {output.merged_vcf_BP_norm} &&
