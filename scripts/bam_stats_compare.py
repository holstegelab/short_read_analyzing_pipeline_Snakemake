...
def compute_bam_stats_py(in_path: str):
    if in_path == '-':
        pipe_in = sys.stdin
    else:
        in_process = subprocess.Popen(['samtools', 'view', '-h', in_path], stdout=subprocess.PIPE, universal_newlines=True)
...
def main():
    ...
    ap.add_argument('-i', required=True, help='input SAM path or - for stdin (use samtools view -h to convert BAM)')
    ...
    if _HAS_FASTCHECK:
        (b1_seq, b1_qual, b1_nrow, b1_nbases,
         b2_seq, b2_qual, b2_nrow, b2_nbases) = fastcheck.sam_stats(args.i)
    else:
        (b1_seq, b1_qual, b1_nrow, b1_nbases,
         b2_seq, b2_qual, b2_nrow, b2_nbases) = compute_bam_stats_py(args.i)
