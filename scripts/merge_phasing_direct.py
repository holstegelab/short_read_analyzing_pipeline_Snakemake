...
def process_record(grecord, vrecord, gphase_blocks, wphase_blocks):
    ...
    #now start matching the ref/alts
    gref = grecord.REF
    vref = refrecord.REF

    if len(gref) > len(vref) and gref.startswith(vref):
        add_to_alt = gref[len(vref):]
        v_allele1 = v_allele1 + add_to_alt
        v_allele2 = v_allele2 + add_to_alt

    allele_idx = {gref:0}
    for pos, alt in enumerate(grecord.ALT):
        allele_idx[alt] = pos + 1

    
    gt1 = allele_idx[v_allele1]
    gt2 = allele_idx[v_allele2]
...
def main():
    ...
    while gvcf_record is not None and vcf_record is not None:
        #if(vcf_record.POS == 29942257):
        #    print(vcf_record.POS, gvcf_record.POS)
        #if(gvcf_record.POS == 29942257):
        ...
        # If the phased VCF record is before the gVCF record, get the next phased VCF record
            ...
            # This does not account for the case where the gVCF record has a longer reference allele than the phased VCF record
